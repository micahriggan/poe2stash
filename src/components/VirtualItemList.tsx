...
export const VirtualItemList: React.FC<VirtualItemListProps> = ({
  items,
  viewMode,
  onPriceClick,
  onRefreshClick,
  ...
  const gridConfig = useMemo(() => {
    if (viewMode === 'grid') {
      ...
      return {
        ...
        itemHeight: 400, // Increased height for list items to prevent overflow
        gap: 0,
      };
    }
  ...
  const virtualItems = useMemo((): VirtualItem[] => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const itemSpacing = viewMode === 'list' ? 32 : gridConfig.gap;
